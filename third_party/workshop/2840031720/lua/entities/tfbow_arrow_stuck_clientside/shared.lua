...
function ENT:TargetEnt( init )
	if self.targent and IsValid(self.targent) then
		if init then
			...
			ent = self.targent
			self.targbone = self.targbone or ent:TranslatePhysBoneToBone(self.targphysbone)
			bone = self.targbone

			if not ent:GetBoneCount() or ent:GetBoneCount() < 1 then
				bonepos = ent:GetPos()
		...
		self:FollowBone( self.targent, self.targbone or 0 )
