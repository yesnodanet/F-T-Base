...
function SWEP:SetupDataTables(...)
	...
	self:NetworkVarTFA("Float", "ShakingSpreadIncrement")
end
...
function SWEP:Deploy(...)
	...
	self:SetShakingSpreadIncrement(0)
...
function SWEP:CalculateRatios()
	BaseClass.CalculateRatios(self)

	local targ = self:GetShaking() and 1 or 0
	local prog = self:GetShakingSpreadIncrement()
	self:SetShakingSpreadIncrement(math.Approach(prog, targ, (targ - prog) * FrameTime() * 5))
end

function SWEP:GetBaseSpread()
	return Lerp(self:GetShakingSpreadIncrement(), BaseClass.GetBaseSpread(self), self:GetStatL("Primary.SpreadShake"))
end
...
local AttachArrowModel = function(a, b, c, wep)
	...
	if b.HitWorld and not (IsValid(b.Entity) and not b.Entity:IsWorld()) then
		...
		arrowstuck.targbone = b.HitBoxBone
		arrowstuck:Spawn()
	end
end
