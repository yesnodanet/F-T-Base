...
function SWEP:SetupDataTables()
	...
	self:AddNetworkVarWatchers()
	if CLIENT then
		self:AddNetworkVarWatchersCL()
	end

	hook.Run("TFA_SetupDataTables", self)
end

function SWEP:AddNetworkVarWatchers()
	self:NetworkVarNotify("Customizing", self.CustomizingUpdated)

	hook.Run("TFA_AddNetworkVarWatchers", self)
end
...
function SWEP:Deploy()
	...
	local IsPlayerOwned = IsValid(ply) and ply:IsPlayer()
	local IsNPCOwned = ply:IsNPC()

	self2.IsNPCOwned = IsNPCOwned
	...
	if IsPlayerOwned then
		...
		if SERVER and self:GetStatL("FlashlightAttachment", 0) > 0 and ply:FlashlightIsOn() then
...
function SWEP:CanSecondaryAttack()
	local attackKeyPressed = false
	if IsValid(self:GetOwner()) and self:GetOwner():IsPlayer() then
		attackKeyPressed = self:GetOwner():KeyPressed(IN_ATTACK2)
	end

	local self2 = self:GetTable()

	local v = hook.Run("TFA_PreCanSecondaryAttack", self)
	...
	if self:GetStatL("Secondary.ClipSize") <= 0 and self:Ammo2() < self:GetStatL("Secondary.AmmoConsumption") then
		Dryfire(self, self2)
		return false
	end
	...
	if self2.GetStatL(self, "Secondary.FiresUnderwater") == false and self:GetOwner():WaterLevel() >= 3 then
	...
	local v2 = hook.Run("TFA_CanSecondaryAttack", self)
	...
	if self:CheckJammed() then
		...
		local typev, tanim = self:ChooseAnimation("shoot2_empty")
