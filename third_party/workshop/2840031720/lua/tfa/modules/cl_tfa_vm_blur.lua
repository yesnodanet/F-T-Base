...
local ShouldSkipVMDraw = TFA.ShouldSkipVMDraw
hook.Add("PreDrawViewModel", "TFA_DrawViewModel", function(vm, plyv, wep, flags)
	if not vm or not plyv or not wep then return end
	if not wep.IsTFAWeapon then return end
	if ShouldSkipVMDraw(vm, wep, flags) then return end
...
hook.Add("PostDrawViewModel", "TFA_DrawViewModel", function(vm, plyv, wep, flags)
	if not wep.IsTFAWeapon then return end
	if ShouldSkipVMDraw(vm, wep, flags) then return end
	...
	if aimingDown and not scoped and wep.__TFA_AimDoFFrame == FrameNumber() then
		...
		if IsValid(hands) and wep.UseHands then
			...
			STOP = true
			local candraw = hook.Run("PreDrawPlayerHands", hands, vm, plyv, wep, flags)
			STOP = false
...
hook.Add("PreDrawPlayerHands", "TFA_DrawViewModel", function(hands, vm, plyv, wep, flags)
	if STOP then return end
	if not wep.IsTFAWeapon then return end
	if not supports then return end
	if ShouldSkipVMDraw(hands, wep, flags) then return end
...
hook.Add("PostDrawPlayerHands", "TFA_DrawViewModel", function(hands, vm, plyv, wep, flags)
	if not wep.IsTFAWeapon then return end
	if ShouldSkipVMDraw(hands, wep, flags) then return end
