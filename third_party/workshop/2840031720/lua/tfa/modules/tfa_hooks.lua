...
local function FixInvalidPMHook()
	...
	if hookTable["PostDrawViewModel"] and hookTable["PostDrawViewModel"]["Set player hand skin"] then
		...
		hook.Add("PreDrawPlayerHands", "Set player hand skin BUT FIXED", function(hands, vm, ply, weapon, flags)
...
if CLIENT then
	...
	hook.Add("PreDrawViewModel", "TFACalculateViewmodel", function(vm, plyv, wepv, flags)
		if lastframe == FrameNumber() then return end
		if not IsValid(wepv) or not wepv.IsTFAWeapon then return end
		if ShouldSkipVMDraw(vm, wepv, flags) then return end
