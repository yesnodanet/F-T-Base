...
Сейчас репозиторий содержит playable вертикальный срез платформы:
...
- модульный runtime со стрельбой, перезарядкой с длительностью, hitscan-уроном,
...
Это ранний, но уже usable фундамент, а не finished production-база оружия.
Advanced projectile physics, persistence и более глубокое animation behavior
остаются точками расширения за существующим IR-контрактом.
...
dispatch. Hitscan-оружие напрямую использует скомпилированные damage, pellets,
spread, tracer, distance falloff, hitgroups, sound, recoil и effects. Более
сложную физическую симуляцию можно развивать за тем же IR-контрактом.
